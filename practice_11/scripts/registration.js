...
document.addEventListener("keydown", (e) => {
  if (e.key === "Enter") {
    if (
      input_login.value === trueLogin &&
      input_passrord.value === truePassword &&
      input_check_pass.value === input_passrord.value &&
      input_login.value.indexOf("@") !== -1 &&
      input_passrord.value.length > 6 &&
      input_passrord.value.indexOf("!" || "_") !== -1
    ) {
      console.log("Вошли");
